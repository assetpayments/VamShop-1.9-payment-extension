...
// class constructor
    function __construct() {
      ...
      $this->code = 'assetpayments';
      $this->title = MODULE_PAYMENT_ASSETPAYMENTS_TEXT_TITLE;
      $this->public_title = MODULE_PAYMENT_ASSETPAYMENTS_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_ASSETPAYMENTS_TEXT_ADMIN_DESCRIPTION;
      $this->icon = DIR_WS_ICONS . 'asset.png';
      $this->sort_order = MODULE_PAYMENT_ASSETPAYMENTS_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_ASSETPAYMENTS_STATUS == 'True') ? true : false);
      $this->form_action_url = 'https://assetpayments.us/checkout/pay';
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_ASSETPAYMENTS_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_ASSETPAYMENTS_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_assetpayments_id'])) {
        $order_id = substr($_SESSION['cart_assetpayments_id'], strpos($_SESSION['cart_assetpayments_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_assetpayments_id']);
        }
      }

      if (vam_not_null($this->icon)) $icon = vam_image($this->icon, $this->title);

      return array(	'module' => $this->public_title,
					'id' => $this->code,
      				'icon' => $icon
                   );
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_assetpayments_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_assetpayments_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_assetpayments_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_ASSETPAYMENTS_TEXT_DESCRIPTION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = '';
        ...
        $order_id = substr($_SESSION['cart_assetpayments_id'], strpos($_SESSION['cart_assetpayments_id'], '-')+1);
        $curr_check = vam_db_query("select currency from " . TABLE_ORDERS . " where orders_id = '" . (int)$order_id . "'");
        $currency = vam_db_fetch_array($curr_check);
		...
		//****Required variables****//	
		$option['TemplateId'] = MODULE_PAYMENT_ASSETPAYMENTS_TEMPLATE_ID;
		$option['CustomMerchantInfo'] = PROJECT_VERSION;
		$option['MerchantInternalOrderId'] = $order_id;
		$option['StatusURL'] = vam_href_link('assetpayments.php', '', 'SSL');	
		$option['ReturnURL'] = vam_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL');
		$option['AssetPaymentsKey'] = MODULE_PAYMENT_ASSETPAYMENTS_ID;
		$option['Amount'] = $order_sum;	
		$option['Currency'] = $currency;
		$option['IpAddress'] = $ip;
		
		//****Customer data and address****//
		$option['FirstName'] = $order->customer['firstname'] . ' ' . $order->customer['lastname'];
        $option['Email'] = $order->customer['email_address'];
        $option['Phone'] = $order->customer['telephone'];
        $option['Address'] = $order->customer['street_address'] . ' ' . $order->customer['city'] . ' ' . $order->customer['postcode'] . ' ' . $order->customer['country']['title'];
		$option['CountryISO'] = $order->customer['country']['iso_code_3'];
		
		//****Adding cart details****//
		for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
        $option['Products'][] = array(
            'ProductId' => vam_get_prid($order->products[$i]['id']),
            'ProductName' => $order->products[$i]['name'],
            'ProductPrice' => $order->products[$i]['price'],
            'ProductItemsNum' => $order->products[$i]['qty'],
		...
		$option['Products'][] = array(
				...
				'ProductItemsNum' => 1,
				'ProductName' => $order->info['shipping_method'],						
				'ProductPrice' => $order->info['shipping_cost'], 					
			);
		
		$merchant_pass = MODULE_PAYMENT_ASSETPAYMENTS_SECRET_KEY; 
		
		$data = base64_encode( json_encode($option) ); 
        
      $process_button_string = vam_draw_hidden_field('data', $data);
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_ASSETPAYMENTS_STATUS'");
    ...
    function install() {
      ...
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_ALLOWED', '', '6', '2', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_ID', '', '6', '3', now())");
	  vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_SECRET_KEY', '', '6', '6', now())");
	  vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_TEMPLATE_ID', '', '6', '6', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_SORT_ORDER', '0', '6', '4', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_ZONE', '0', '6', '5', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_ORDER_STATUS_ID', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
      vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_ASSETPAYMENTS_ORDER_STATUS_FAIL', '0', '6', '7', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_ASSETPAYMENTS_STATUS', 'MODULE_PAYMENT_ASSETPAYMENTS_ALLOWED', 'MODULE_PAYMENT_ASSETPAYMENTS_ID', 'MODULE_PAYMENT_ASSETPAYMENTS_SECRET_KEY', 'MODULE_PAYMENT_ASSETPAYMENTS_TEMPLATE_ID', 'MODULE_PAYMENT_ASSETPAYMENTS_SORT_ORDER', 'MODULE_PAYMENT_ASSETPAYMENTS_ZONE', 'MODULE_PAYMENT_ASSETPAYMENTS_ORDER_STATUS_ID', 'MODULE_PAYMENT_ASSETPAYMENTS_ORDER_STATUS_FAIL');
    }
